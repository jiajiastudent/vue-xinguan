...
<!--                        发放表单-->
                        <el-form size="mini" ref="addRuleFormRef" :rules="addRules" :model="addRuleForm" label-width="80px">
                            ...
                            <el-form-item v-if="existence==1" label="物资去向" prop="consumerId">
                                <el-col :span="12">
                                    <div class="grid-content bg-purple">
                                        <el-select v-if="existence==1" style="width:100%;" filterable @change="consumerSelectChange(addRuleForm.consumerId)"
                                                   v-model="addRuleForm.consumerId" placeholder="选择已存在去向">
                                            <el-option v-for="item in consumers" :key="item.id" :label="item.name" :value="item.id"></el-option>
                                        </el-select>
                                    </div>
                                </el-col>
                            </el-form-item>
                            <el-form-item label="优先级" prop="type">
                                <el-radio-group v-model="addRuleForm.priority">
                                    <el-radio  :size="'mini'" :label="1">不急</el-radio><el-radio  :size="'mini'" :label="2">常规</el-radio>
                                    <el-radio  :size="'mini'" :label="3">紧急</el-radio><el-radio  :size="'mini'" :label="4">特急</el-radio>
                            ...
                            <div v-if="existence==0">
                                ...
                                <el-form-item label="经纬度" prop="ponit">
                                    <el-col :span="8">
                                    <el-input placeholder="请输入经度" v-model="addRuleForm.lng" ></el-input>
                                    </el-col>
                                    <el-col :span="8">
                                    <el-input placeholder="请输入纬度" v-model="addRuleForm.lat"></el-input>
                                    </el-col>
                                    <el-button size="small" type="success" icon="el-icon-circle-plus-outline" @click="addPointDialogVisible=true">选择地点</el-button>
                                </el-form-item>
                            ...
                            <div v-if="existence==1">
                                <el-card class="box-card" style="margin-bottom: 30px;">
                                    <div  class="text item" style="font-size: 12px;padding: 5px;">物资名称: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{consumerInfo.name}}</el-tag>
                  </span>
                                    </div>
                                    <div  class="text item" style="font-size: 12px;padding: 5px;">物资地址: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{consumerInfo.address}}
                    </el-tag></span>
                                    </div>
                                    <div  class="text item" style="font-size: 12px;padding: 5px;">联系方式: <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{consumerInfo.phone}}
                    </el-tag></span>
                                    </div>
                                    <div  class="text item" style="font-size: 12px;padding: 5px;">联系人员:  <span style="margin-left: 200px;color: #999"><el-tag size="mini" type="info">{{consumerInfo.contact}}
                    </el-tag></span>
                                    </div>
                                </el-card>
                            </div>
            ...
            <el-col :span="13">
                <div class="grid-content bg-purple-light">
                    <el-card>
                        ...
                        <el-alert title="勾选先下方物资后,在左侧表单的明细中添加其发放数量" type="warning" show-icon style="margin-bottom:20px;">
                        </el-alert>
                        <!-- 发放预选表格 -->
                        <el-table
                                style="height:420px;"
                        ...
                        <!-- 表格分页 -->
                        <el-pagination
                                background style="margin-top:10px;" @size-change="handleSizeChange" @current-change="handleCurrentChange" :current-page="queryMap.pageNum"
                                :page-sizes="[6, 8, 16, 20]" :page-size="queryMap.pageSize" layout="total, sizes, prev, pager, next" :total="total"
                        ></el-pagination>
                        <!-- 抽屉 -->
     ...
     <el-button size="mini" style="float: right;margin: 10px;"  type="primary" icon="el-icon-remove" :disabled="products.length==0" @click="removeAllItem" >
        ...
        <el-dialog title="添加地点经纬度" :visible.sync="addPointDialogVisible" width="80%"  @close="closePointAddDialog">
            <!--地图-->
            <div class="map">
                        <el-button  v-model="addRuleForm.lng">经度：{{addRuleForm.lng}}</el-button>
                        <el-button  v-model="addRuleForm.lat" >纬度：{{addRuleForm.lat}}</el-button>
                <el-input v-model="addressKeyword" placeholder="请输入地址来直接查找相关位置"></el-input>
                <baidu-map
                        class="bmView" :scroll-wheel-zoom="true" :center="center" :zoom="zoom" @click="getLocationPoint"
            ...
            <span slot="footer" class="dialog-footer">
              <el-button @click="addPointDialogVisible = false">取 消</el-button>
    ...
    export default {
        data() {
            ...
            return {
                ...
                addRules: {
                    consumerId: [{ required: true, message: "请选中物资去向", trigger: "blur" }],
                    type: [{ required: true, message: "请选中领取类型", trigger: "blur" }],
                    priority: [{ required: true, message: "请选中发放类型", trigger: "blur" }],
                    remark: [{ required: true, message: "请输入商品说明备注", trigger: "blur" },
                        { min: 5, max: 20, message: "长度在 5 到 20 个字符", trigger: "blur" }],
                    name: [{ required: true, message: "请输入去向名称", trigger: "blur" },
        ...
        methods:{
            ...
            //得到经纬度
            getLocationPoint(e) {
                this.addRuleForm.lng = e.point.lng;
                this.addRuleForm.lat = e.point.lat;
                //alert(this.ponit.lng+"     "+this.ponit.lat)
                this.$confirm("经度为："+this.addRuleForm.lng+"纬度为："+this.addRuleForm.lat, "提示", {
                ...
                this.$refs.addRuleFormRef.validate(async valid => {
                    if (!valid) {
                        ...
                        this.addRuleForm.address =
                            this.addRuleForm.province + "/" + this.addRuleForm.city + "/" + this.addRuleForm.origin;
                        var car = [];
                        this.products.forEach(row => {
                            if (row.number != undefined) {
                                var item = { productId: row.id, productNumber: row.number,weight: row.weight };
                                car.push(item);
                            }
                        });
                        if (car.length > 0) {
